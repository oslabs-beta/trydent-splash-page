---
title: context-script.js
---

The `content-script.js` file is responsible for listening to user interactions with the web page, extracting information about the target elements, and sending that information to the extension's background script and the Chrome DevTools panel.

## Overview
The script listens for the following events: 'click', 'focus', 'blur', and 'change'. When an event is detected, it captures the event's details, including the target element's XPath, and sends the information to the background script and the DevTools panel. This information is used to generate Cypress test code for the user interactions.

## Functions

### getRelativeXPath(element)

This function takes an HTMLElement as an input and returns its relative XPath. The function prioritizes unique attributes (such as 'data-cy', 'data-test', 'data-testid', and 'id') when generating the XPath. If none of the unique attributes are present, the function calculates the element's position among its siblings with the same tag name. If the element is an anchor tag with an `href` attribute, the function updates global variables to indicate this.
```javascript
...
This function serves as an event listener for input events like 'click', 'focus', 'blur', and 'change'. It takes an event object and a callback function as arguments. The function extracts relevant information from the event object (such as the target element's XPath and the event type) and then calls the callback function with the extracted data.
```javascript
/**
...
```
## Event Listeners
The script sets up event listeners for 'click', 'focus', 'blur', and 'change' events. For each event, it calls the `inputEventListener()` function, which in turn sends messages with the event details to the background script and the DevTools panel.
```javascript
