  ...
  return (
    <>
      <section className="p-10 mt-10 grid lg:grid-cols-2 lg:gap-2 place-items-center " style={{ background: 'radial-gradient(at -800% -400%, #f6f6f6, #072854)' }}>
        ...
        <div
          className={` ${isClicked ? ' flex justify-center items-center min-h-screen fixed top-0 left-0 w-full h-full bg-black bg-opacity-75 z-50' : ''}`}
          onClick={handleClick}
        >
          <img src={gif}
            className={`transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${isClicked ? 'scale-150 shadow-lg hover:scale-150' : ''}`}
            onClick={handleClick}
          />
        </div>
      </section>
